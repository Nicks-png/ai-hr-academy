...
router.post('/api/candidatos/submit', (req, res) => {
  ...
  // Normaliza: adiciona 55 se número nacional (10 ou 11 dígitos)
  const phone = (digits.length === 10 || digits.length === 11) ? '55' + digits : digits

  // Duplicata
  const existing = db.prepare('SELECT id FROM candidates WHERE phone = ?').get(phone)
  if (existing)
